...
- Supabase Auth, Google 소셜 로그인 기능, 세션 처리, RLS 정책 적용 계획 수립
- 웹 전용 레이아웃 구조 추가
...
| REQ-2 | 앱 로그인 기능 추가 및 세션 처리 | Google 소셜 로그인이 동작하고 미로그인 사용자는 로그인 화면으로 이동하며 로그인 사용자는 앱 화면 접근 |
...
- Google 소셜 로그인 성공 확인
- Google 로그인 취소 또는 실패 시 오류/취소 상태 표시 확인
