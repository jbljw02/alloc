...
import { AllocationHistoryFilter, AllocationHistoryItem } from '@/hooks/history/allocationHistory';
...
const getItemColor = (item: AllocationHistoryItem): string => {
  return item.assetColor ?? CATEGORY_CONFIG[item.category].color;
};

const getItemIconName = (item: AllocationHistoryItem): keyof typeof Ionicons.glyphMap => {
  return item.assetIconName ?? CATEGORY_CONFIG[item.category].icon;
};

export const HistoryItemList = ({ items, selectedFilter, selectedMonthLabel }: HistoryItemListProps) => {
  if (items.length === 0) {
    return (
      <View className="bg-white rounded-2xl p-6 items-center">
        <Text className="text-base font-semibold text-gray-800 mb-1">아직 배분 내역이 없습니다.</Text>
        <Text className="text-sm text-gray-500">{getEmptyMessage(selectedFilter, selectedMonthLabel)}</Text>
      </View>
    );
  }
